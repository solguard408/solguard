...
import { generateRiskSummary, OPENROUTER_MODEL } from "./aiSummary";
import { buildReport, finding, formatSol, impactFromScore, holderConcentrationFinding, holderShareLabel } from "./reportBuilder";
...
function buildOnChainVerdict(scan) {
  ...
  return `This token scores ${scan.riskScore}/100 (${scan.riskLevel}) with mint authority ${scan.authorityCheck.mintAuthority.toLowerCase()}, freeze authority ${scan.authorityCheck.freezeAuthority.toLowerCase()}, and top 10 holders at ${top10} — ${flag}.`;
}
...
async function execTokenAudit({ tokenAddress }) {
  ...
  const verdict = ai.aiAvailable && ai.verdict ? ai.verdict : buildOnChainVerdict(scan);
  return buildReport({
    agentId: "token-audit",
    input: tokenAddress,
    ...
    confidence: ai.aiAvailable ? `${tokenConfidence(scan)}; AI verdict generated` : `${tokenConfidence(scan)}; AI unavailable, on-chain verdict used`,
    dataSource: TOKEN_SOURCES,
    scannedAt: scan.scannedAt,
    rawEvidence: { ...scan, ai_summary_available: ai.aiAvailable, ai_summary_reason: ai.reason },
    ai_summary_available: ai.aiAvailable,
    ai_summary_reason: ai.reason,
  });
}
...
async function execRugProbability({ tokenAddress }) {
  ...
  return buildReport({
    ...
    keyFindings: tokenFindingsFromScan(scan),
    recommendations: deriveTokenRecs(scan),
    confidence: ai.aiAvailable ? "High — AI synthesis over full on-chain scan" : "Medium — on-chain factors only, AI unavailable",
...
async function execRiskScore({ tokenAddress }) {
  const scan = await getCachedScan(tokenAddress);
  const verdict = `Composite weighted score is ${scan.riskScore}/100 (${scan.riskLevel}) across mint/freeze authorities, holder concentration, bundle signals, and liquidity.`;
  return buildReport({
    agentId: "risk-score",
    input: tokenAddress,
    riskScore: scan.riskScore,
    ...
    try {
      ...
      if (text) {
        ...
        const recommendations = recMatch
          ...
          : ["Apply this guidance alongside an on-chain scan before making financial decisions.", "Verify any token or wallet mentioned using SolGuard's automated agents."];
        return buildReport({
          ...
          verdict,
          keyFindings: [
            finding("Consultation Topic", query.slice(0, 120) + (query.length > 120 ? "…" : ""), "neutral", "Question submitted for AI security analysis."),
            finding("Full Response Length", `${text.split(/\s+/).length} words`, "neutral", "Detailed guidance is available in the raw response below."),
          ],
          recommendations: recommendations.slice(0, 4),
          confidence: "Medium — AI-generated guidance; verify with on-chain scans",
          dataSource: AI_SOURCES,
          rawEvidence: { question: query, fullResponse: text, ai_summary_available: true },
          ai_summary_available: true,
          ai_summary_reason: null,
        });
      }
      lastError = new Error("Empty AI response from OpenRouter");
    } catch (e) {
      lastError = e;
      logConsultantError(e, { attempt });
      if (attempt < 2) await new Promise((r) => setTimeout(r, 800));
    }
  }

  logConsultantError(lastError, { final: true });
  throw new Error(`AI consultant unavailable: ${lastError?.message || "OpenRouter request failed"}`);
}
...
function logConsultantError(e, extra = {}) {
  console.error("[OpenRouter] AI consultant failed:", {
